...
class UpdateReceptionistRequest extends FormRequest
{
    public function authorize(): bool
    {
        ...
        if (! $receptionist instanceof User) {
            return false;
        }

        return ($this->user()?->hasRole('Admin') ?? false) && $receptionist->hasRole('Receptionist');
    }
    ...
    public function rules(): array
    {
        return [
            ...
            'email' => ['required', 'email', Rule::unique('users', 'email')->ignore($this->route('receptionist'))],
