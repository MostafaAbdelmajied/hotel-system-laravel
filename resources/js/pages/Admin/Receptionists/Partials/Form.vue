...
type Props = {
    form: InertiaForm<ReceptionistFormData>;
    countries: string[];
    submitLabel: string;
    cancelHref: string;
    ...
    <form class="space-y-6" @submit.prevent="$emit('submit')">
        <div class="grid gap-4 md:grid-cols-2">
            <div class="grid gap-2">
                ...
                <Input id="name" v-model="name" autocomplete="name" name="name" placeholder="Receptionist name" />
                <InputError :message="form.errors.name" />
            </div>

            <div class="grid gap-2">
                <Label for="email">Email</Label>
                <Input id="email" v-model="email" autocomplete="email" name="email" placeholder="receptionist@example.com" type="email" />
        ...
        <div class="rounded-lg border border-sidebar-border/70 p-4 dark:border-sidebar-border">
            <div class="mb-4">
                ...
                <p class="text-sm text-muted-foreground">
                    {{ showPasswordFields ? 'Set the password for the new receptionist account.' : 'Leave blank to keep the current password.' }}
                </p>
            </div>
